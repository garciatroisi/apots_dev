...
function filterMintForTransactions(transactions: any[]): any[] {
  console.log("Filtering transactions that use mint_for function...");
...
async function main() {
  try {
    ...
    // Filter only mint_for transactions
    const mintForTransactions = filterMintForTransactions(allTransactions);

    // Save to file if there are mint_for transactions
    if (mintForTransactions.length > 0) {
      const fs = require("fs");
      const timestamp = Date.now();
      const filename = `mint-for-transactions-${TARGET_ACCOUNT.substring(
        0,
        10
      )}-${timestamp}.json`;
      ...
      console.log("\nFirst 3 mint_for transactions:");
      mintForTransactions.slice(0, 3).forEach((tx, index) => {
        const version = tx.version || "N/A";
        const hash = tx.hash ? tx.hash.substring(0, 10) + "..." : "N/A";
        const functionName = tx.payload?.function || "N/A";

        console.log(
          `  ${
            index + 1
          }. Version: ${version}, Hash: ${hash}, Function: ${functionName}`
        );
      });
    } else {
