...
const TRANSACTION_HASH =
  "0x65c8ee51263124173f6e62e18e93c27e6f8aed260bcd3a3885c6c9ff9e792a7b";
...
async function analyzeTransactionTokens(
  ...
  try {
    ...
    // Wait for transaction and get details
    ...
    if ("version" in txnDetails) {
      console.log(`📊 Transaction version: ${txnDetails.version}`);
    }

    if ("timestamp" in txnDetails) {
    ...
    const results = {
      transactionHash: transactionHash,
      transactionVersion:
        "version" in txnDetails ? txnDetails.version : undefined,
      timestamp: "timestamp" in txnDetails ? txnDetails.timestamp : undefined,
      success: "success" in txnDetails ? txnDetails.success : undefined,
...
// Function to get transaction by hash without waiting
async function getTransactionByHash(transactionHash: string): Promise<void> {
  try {
    console.log(`🔍 Getting transaction by hash: ${transactionHash}`);
...
async function main() {
  try {
    ...
    try {
      ...
      await analyzeTransactionTokens(TRANSACTION_HASH);
    }
  } catch (error) {
